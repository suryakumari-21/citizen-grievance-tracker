...
export default function ComplaintDetail() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        console.error("Error loading complaint:", err.response?.data || err.message);
        // If not found or forbidden, show message and allow back
  ...
  const postComment = async () => {
    ...
    try {
      ...
      // backend returns { msg, complaint } — update local complaint
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <div className="text-sm text-gray-500 mt-1">
          {complaint.category} • {new Date(complaint.createdAt).toLocaleString()}
        </div>

        <div className="mt-4">
          <span className={`px-3 py-1 rounded-full text-sm ${
            complaint.status === "Pending" ? "bg-yellow-100 text-yellow-700" :
            complaint.status === "In Progress" ? "bg-orange-100 text-orange-700" : "bg-green-100 text-green-700"
          }`}>
            {complaint.status}
          </span>
        </div>
              ...
              {complaint.comments.map((c, i) => (
                <li key={i} className="bg-gray-50 p-3 rounded">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    — {c.user?.name || "User"} • {new Date(c.date).toLocaleString()}
                  </div>
                </li>
              ))}
            </ul>
